            ...
            if (eve.pId === ev._id) {
              ...
              return (
                <div key={eve.pId} className="d-flex">
                  {loadingdel ?  <p className="loading2" style={{ fontSize: "40px", width: "100px" }}/> :  <button onClick={() => deleteHandler(eve.pId)}  className="btn" > Remove </button>}
                    <div id="j-c">
                      <ButtonGroup>
                  ...
                  <div className="color-global" style={{ backgroundColor: eve.color }}/> &nbsp; <p className="py-1">{eve.color}</p> </div>
                )
              }
            })
:
  ...
  return (
    <div>
      ...
      {user && data.length > 0 && (
        <div
          className="border border-danger rounded-pill text-center p-2"
          style={{ position: "sticky" }}
        >
          {" "}
          <Link to="/order" id="no-a" className="text-warning h3">
            Continue to order
          </Link>
        </div>
      )}
      {!user && data.length > 0 && (
        ...
        >
          {" "}
          <a href="/login" id="no-a" className="text-warning h3">
            Login to continue
          </a>
        </div>
      )}
